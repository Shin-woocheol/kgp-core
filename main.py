...
parser.add_argument('--algorithm', default='kgpcore',
                    help='specify algorithm name')

parser.add_argument('--network', default="../dataset/real/congress/network.hyp",
...
if args.mode == 'single':
    ...
    if args.algorithm == 'ASAP':
        ...
        print(f"Run time: {time.time() - start_time:.4f}")
        exit()
    #python main.py --mode single --k 5 --g 5 --p 0.7  --algorithm naive --network ./dataset/real/contact/network.hyp
...
#python main.py --mode compare --network ./dataset/real/contact/network.hyp
if args.mode == 'compare':
    ...
    for i in range(1,11):
        for j in range(1,11):
            for k in [x * 0.1 for x in range(0, 11)]:
                tot += 1
...
#python main.py --k 4 --g 4 --p 0.5 --algorithm naive --network ./dataset/real/contact/network.hyp
#결과파일 저장 코드
output_dir = 'output'
